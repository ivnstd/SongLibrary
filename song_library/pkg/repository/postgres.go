...
func NewDB(cfg Config) (*gorm.DB, error) {
	...
	db.AutoMigrate(&models.Artist{}, &models.Song{})
...
func seedData(db *gorm.DB, data interface{}, tableName string) {
	if err := db.Create(data).Error; err != nil {
		logrus.Fatalf("Failed to seed %s: %s", tableName, err.Error())
	} else {
		logrus.Infof("%s seeded successfully", tableName)
	}
}
...
func SeedDatabase(db *gorm.DB) {
	...
	seedData(db, &artists, "artists")
	seedData(db, &songs, "songs")
}
